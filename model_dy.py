...
class GCN(nn.Module): 
    def __init__(self, in_ft, out_ft, act, bias=True, nhead=2): 
        super(GCN, self).__init__()
        self.fc = nn.Linear(in_ft, out_ft, bias=False)
        self.act = self.get_activation(act)
        self.adj_transform = nn.Linear(8, 5, bias=False)  
        if bias:
        ...
        for m in self.modules():
            self.weights_init(m)
        self.transformer = nn.MultiheadAttention(out_ft, nhead)
...
class DynamicWeightModule(nn.Module):
    ...
    def forward(self, h_gcn, h_transformer):
        concat_features = torch.cat((h_gcn, h_transformer), dim=-1)
        weights = self.fc(concat_features)
...
class Model(nn.Module):
    def __init__(self, n_in, n_h, activation, negsamp_round_patch, negsamp_round_context, readout):
        ...
        self.dynamic_weight = DynamicWeightModule(n_h)
    ...
    def forward(self, seq1, adj, sparse=False, msk=None, samp_bias1=None, samp_bias2=None):
        ...
        weights = self.dynamic_weight(h_1[:, -1, :], h_2[:, -1, :])
        weighted_h_1 = weights[:, 0].unsqueeze(1).unsqueeze(2) * h_1
        weighted_h_2 = weights[:, 1].unsqueeze(1).unsqueeze(2) * h_2
        h_combined = weighted_h_1 + weighted_h_2
